...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (platform === 'hackthebox') {
      ...
      // 2. Season ranking — correct HTB v4 endpoint
      // Tries the actual endpoints HTB frontend uses for season data
      ...
      const seasonEndpoints = [
        // HTB frontend calls this for the season leaderboard widget
        'https://labs.hackthebox.com/api/v4/season/end-user/ranking/3175444',
        'https://labs.hackthebox.com/api/v4/rankings/season/3175444',
        'https://labs.hackthebox.com/api/v4/user/season/3175444',
        'https://labs.hackthebox.com/api/v4/season/ranking/user/3175444',
        'https://labs.hackthebox.com/api/v4/season/user/3175444',
        // Profile overview sometimes has season embedded
        'https://labs.hackthebox.com/api/v4/user/profile/overview/3175444',
      ];

      for (const url of seasonEndpoints) {
        try {
          const sr = await fetch(url, { headers });
          console.log(`[HTB] ${url} → ${sr.status}`);
          if (sr.ok) {
            const sd = await sr.json();
            console.log(`[HTB] SUCCESS ${url}:`, JSON.stringify(sd).slice(0, 1000));
            const s = sd.data ?? sd.profile ?? sd;

            // Extract season rank
            if (s.rank != null)      seasonRank = `#${s.rank}`;
            if (s.position != null)  seasonRank = `#${s.position}`;
            if (s.ranking != null)   seasonRank = `#${s.ranking}`;

            // Extract tier
            if (s.tier)        seasonTier = s.tier;
            if (s.leagueTier)  seasonTier = s.leagueTier;
            if (s.league_tier) seasonTier = s.league_tier;
            if (s.badge)       seasonTier = s.badge;

            // Extract season points
            if (s.points != null)        seasonPts = s.points;
            if (s.total_points != null)  seasonPts = s.total_points;
            if (s.season_points != null) seasonPts = s.season_points;

            // Extract flags
            if (s.user_owns != null)  ownedFlags = s.user_owns;
            if (s.owned != null)      ownedFlags = s.owned;
            if (s.total != null)      totalFlags = s.total;
      ...
      const result = {
        ...
        level:      p.level        ?? 28,
        // Season (Apprentice / Ruby / #811 / 350pts / 12/26)
        seasonRank,
        seasonTier,
        seasonPts,
        flags:      ownedFlags,
      ...
      console.log('[HTB] final:', JSON.stringify(result));
